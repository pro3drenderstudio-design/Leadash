...
csv.field_size_limit(10 * 1024 * 1024)  # 10MB limit for large fields
...
O_FUNDING     = 43
O_FUND_STAGE  = 44
O_LINKEDIN    = None  # not directly in org file
O_SOURCE_ID   = 46   # _id
...
def parse_size(raw):
    if not raw:
        return None
    # raw is like "['5']" or "['3', '4']"
    import re
    tags = re.findall(r"'(\d+)'", raw)
    if tags:
        return SIZE_TAG_MAP.get(tags[0])
    return None
...
def import_orgs(filepath):
    ...
    with open(filepath, encoding="utf-8", errors="replace") as f:
        ...
        for i, row in enumerate(reader):
            ...
            size     = parse_size(safe(row, O_SIZE_TAG))
