...
async function checkDomainDns(domain: string, expected: DnsRecord[]): Promise<{ ok: boolean; failures: string[] }> {
...
export async function runInboxDnsHealth(): Promise<void> {
  ...
  for (const row of domains) {
    ...
    checked++;
    const { ok, failures } = await checkDomainDns(row.domain, expected);
    ...
    if (!ok) {
      ...
      if (shouldAlert) {
        ...
        if (toEmail) {
          ...
          await sendInboxDnsAlertEmail({ to: toEmail, domain: row.domain, failures })
