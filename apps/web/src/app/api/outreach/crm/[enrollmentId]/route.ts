...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ enrollmentId: string }> }) {
  ...
  const body = await req.json() as { crm_status?: string; is_starred?: boolean; remind_at?: string | null; scheduled_reply_at?: string | null; scheduled_reply_body?: string | null; crm_labels?: string[] };
  ...
  if (body.crm_labels          !== undefined) patch.crm_labels          = body.crm_labels;
