  ...
  return (
    <div className="rounded-2xl border border-white/[0.07] bg-white/[0.03] backdrop-blur-sm p-8 shadow-[0_0_0_1px_rgba(255,255,255,0.04)_inset]">
      <div className="mb-7 text-center">
        ...
        <p className="mt-1 text-sm text-white/40">Sign in to your Leadash account</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <Link href="/forgot-password" className="text-xs text-white/35 hover:text-white/60 transition-colors">Forgot?</Link>
          </div>
          <div className="relative">
            <input
              type={showPassword ? "text" : "password"} required value={password}
              onChange={e => setPassword(e.target.value)}
              className="w-full rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white outline-none pr-10 transition-colors focus:border-blue-500/60 focus:bg-white/7"
            />
            <button
      ...
      <p className="mt-6 text-center text-xs text-white/30">
        ...
        <Link href="/signup" className="text-blue-400 hover:text-blue-300 font-medium transition-colors">Create one free</Link>
      </p>
    </div>
  );
}
