...
/**
 * Lightweight adaptive HLS video player — the same optimization used by the
 * funnel builder's video block, factored out for reuse.
 *
 *  - Streams HLS via hls.js (native HLS on Safari/iOS), so it adapts bitrate to
 *    the viewer's bandwidth instead of downloading one heavy MP4.
 *  - `capLevelToPlayerSize` never fetches a rendition larger than the on-screen
 *    player — big bandwidth win on small/embedded players.
 *  - `startLevel: -1` + lazy `preload="none"` = fast start, no wasted data.
 *  - A quality menu lets the viewer pin a rendition; default is Auto.
 *
 * Works with Mux signed playback: pass `https://stream.mux.com/<id>.m3u8?token=<jwt>`
 * — Mux embeds the token in the child manifests so segment auth just works.
 */
interface Props {
  src: string;
  poster?: string;
  /** Fired on timeupdate with the watched percentage (0–100). */
...
export function AdaptiveVideo({ src, poster, onProgress }: Props) {
  ...
  useEffect(() => {
    ...
    // Safari / iOS play HLS natively — no JS-level quality control there.
    if (video.canPlayType("application/vnd.apple.mpegurl")) {
      ...
      import("hls.js").then(({ default: Hls }) => {
        ...
        hls.on(Hls.Events.LEVEL_SWITCHED, (_evt, data) => setActiveLevel(data.level));
      });
    }
