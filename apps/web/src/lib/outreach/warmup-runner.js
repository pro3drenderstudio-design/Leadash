...
exports.runWarmupPool = runWarmupPool;
exports.runWarmupRamp = runWarmupRamp;
const supabase_js_1 = require("@supabase/supabase-js");
const gmail_1 = require("@/lib/outreach/gmail");
const microsoft_1 = require("@/lib/outreach/microsoft");
const smtp_1 = require("@/lib/outreach/smtp");
const warmup_templates_1 = require("@/lib/outreach/warmup-templates");
...
function supabase() {
    return (0, supabase_js_1.createClient)((process.env.NEXT_PUBLIC_SUPABASE_URL ?? process.env.SUPABASE_URL), process.env.SUPABASE_SERVICE_ROLE_KEY);
}
async function runWarmupPool(workspaceId) {
    const db = supabase();
    const result = { sent: 0, replied: 0, rescued: 0 };
    // ── Trial expiry guard ──────────────────────────────────────────────────────
    ...
    const localPool = (globalPool ?? []).filter((i) => i.workspace_id === workspaceId);
    if (!localPool.length || !globalPool || globalPool.length < 2)
        return result;
    ...
    for (const sender of localPool) {
        ...
        if (toSend === 0)
            continue;
        const recipients = globalPool.filter((r) => r.id !== sender.id);
        if (!recipients.length)
            continue;
        for (let i = 0; i < toSend; i++) {
            const recipient = recipients[i % recipients.length];
            const warmupId = crypto.randomUUID();
            const template = (0, warmup_templates_1.selectSendTemplate)(`${sender.id}-${recipient.id}-${Date.now()}`, sender.first_name, recipient.first_name);
            ...
            try {
                ...
                if (sender.provider === "gmail" && sender.oauth_refresh_token) {
                    const r = await (0, gmail_1.sendGmailMessage)(sender, { to: recipient.email_address, subject: template.subject, htmlBody, textBody: template.body, customHeaders: warmupHdr });
                    messageId = r.messageId;
                    threadId = r.threadId ?? "";
                }
                else if (sender.provider === "outlook" && sender.oauth_refresh_token) {
                    const r = await (0, microsoft_1.sendMicrosoftMessage)(sender, { to: recipient.email_address, subject: template.subject, htmlBody, textBody: template.body, customHeaders: warmupHdr });
                    messageId = r.messageId;
                    threadId = r.threadId ?? "";
                }
                else {
                    const r = await (0, smtp_1.sendSmtpMessage)(sender, { to: recipient.email_address, subject: template.subject, htmlBody, textBody: template.body, customHeaders: warmupHdr });
                    messageId = r.messageId;
                }
    ...
    for (const ws of pending ?? []) {
        ...
        const recipient = localPool.find((p) => p.id === ws.to_inbox_id);
        // sender can be any inbox in the global pool (we only need their email address)
        const sender = globalPool.find((p) => p.id === ws.from_inbox_id);
        if (!recipient || !sender)
            continue;
        const replyWarmupId = crypto.randomUUID();
        const tpl = (0, warmup_templates_1.selectReplyTemplate)(`reply-${ws.id}`, sender?.first_name);
        ...
        try {
            if (recipient.provider === "gmail" && recipient.oauth_refresh_token) {
                await (0, gmail_1.sendGmailMessage)(recipient, { to: sender.email_address, subject: subj, htmlBody: html, textBody: tpl.body, replyToThreadId: ws.thread_id, inReplyToMessageId: ws.message_id, customHeaders: hdr });
            }
            else if (recipient.provider === "outlook" && recipient.oauth_refresh_token) {
                await (0, microsoft_1.sendMicrosoftMessage)(recipient, { to: sender.email_address, subject: subj, htmlBody: html, textBody: tpl.body, replyToThreadId: ws.thread_id, inReplyToMessageId: ws.message_id, customHeaders: hdr });
            }
            else {
                await (0, smtp_1.sendSmtpMessage)(recipient, { to: sender.email_address, subject: subj, htmlBody: html, textBody: tpl.body, inReplyToMessageId: ws.message_id, customHeaders: hdr });
            }
    ...
    for (const ws of recentSends ?? []) {
        const inbox = localPool.find((p) => p.id === ws.to_inbox_id);
        if (!inbox)
            continue;
        try {
            let rescued = false;
            if (inbox.provider === "gmail" && inbox.oauth_refresh_token) {
                rescued = await rescueGmail(inbox, ws.id);
            }
            else if (inbox.imap_host) {
                rescued = await rescueSmtp(inbox, ws.id);
            }
            if (rescued) {
...
// ── Gmail spam rescue ──────────────────────────────────────────────────────────
async function rescueGmail(inbox, warmupId) {
    const { getGmailClient } = await Promise.resolve().then(() => __importStar(require("@/lib/outreach/gmail")));
...
// ── SMTP spam rescue ───────────────────────────────────────────────────────────
async function rescueSmtp(inbox, warmupId) {
    const { ImapFlow } = await Promise.resolve().then(() => __importStar(require("imapflow")));
    const { decrypt } = await Promise.resolve().then(() => __importStar(require("@/lib/outreach/crypto")));
    const pass = inbox.smtp_pass_encrypted ? decrypt(inbox.smtp_pass_encrypted) : "";
    const client = new ImapFlow({ host: inbox.imap_host, port: inbox.imap_port ?? 993, secure: true, auth: { user: inbox.smtp_user, pass }, logger: false, connectionTimeout: 8000, greetingTimeout: 5000, socketTimeout: 15000 });
    ...
    for (const folder of folders) {
        ...
        try {
            for await (const msg of client.fetch({ since: new Date(Date.now() - 86400000) }, { envelope: true, headers: ["x-ld-ref"] })) {
                const h = msg.headers?.toString() ?? "";
                const m = h.match(/x-ld-ref:\s*(.+)/i);
                if (m?.[1]?.trim() !== warmupId)
                    continue;
                if (msg.uid) {
...
// ── Daily ramp (+1/day until target) ─────────────────────────────────────────
async function runWarmupRamp(workspaceId) {
    const db = supabase();
    ...
    for (const inbox of inboxes ?? []) {
        if (inbox.warmup_current_daily >= inbox.warmup_target_daily)
            continue;
        ...
        const daysLeft = inbox.warmup_ends_at
            ? Math.max(1, Math.ceil((new Date(inbox.warmup_ends_at).getTime() - now) / 86400000))
            : 1;
